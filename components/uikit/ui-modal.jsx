...
export function UiModal({width = "md", className, children, isOpen = false, onClose}) {
    ...
    const modal = (
        <div onClick={handleClick} className={clsx("fixed inset-0 bg-slate-900/60 backdrop-blur pt-10 pb-10 overflow-y-auto", className)}>
            <div data-id="modal" className={clsx("bg-white rounded-lg min-h-[320px] mx-auto relative flex flex-col", {
                md: 'max-w-[640px] w-full',
                full: ' mx-5'
            }[width])}>
                <button onClick={onClose} className="w-8 h-8 flex items-center justify-center bg-white/10 absolute top-0 left-[calc(100%+12px)] rounded">
